...
let db = null;
let initPromise = null;
const DB_FILENAME = 'tea_school_final_v2.db'; // V2 forces a clean reset for the new schema
...
export const initDatabase = async () => {
  ...
  initPromise = (async () => {
    ...
    // COMPLETE SCHEMA (One clean block matching Electron main.js)
    db.exec(`
      CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT);
      CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT DEFAULT 'admin', created_at TEXT DEFAULT (datetime('now')));
...
export const pwaApi = {
  ...
  createStudent: async (s) => {
    ...
    const id = await sqlRun(
      `INSERT INTO students (admission_number, first_name, middle_name, last_name, class, section, guardian_name, guardian_phone, status) 
       VALUES (?,?,?,?,?,?,?,?,?)`,
      [admissionNumber, s.first_name, s.middle_name || '', s.last_name, s.class, s.section || '', s.guardian_name || '', s.guardian_phone || '', 'active']
    );
  ...
  updateStudent: (s) => sqlRun(`
    UPDATE students SET 
      ...
      section=?, guardian_name=?, guardian_phone=?, status=? 
    WHERE id=?`,
    [s.first_name, s.middle_name || '', s.last_name, s.class, s.section, s.guardian_name, s.guardian_phone, s.status, s.id]
  ),
  deleteStudent: (id) => sqlRun("UPDATE students SET status = 'inactive' WHERE id = ?", [id]),
