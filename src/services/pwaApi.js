...
//  Persistence: database exported as base64 → localStorage after every write
//  Works on iOS Safari, Android Chrome — no OPFS / SharedArrayBuffer needed
//
//  Setup:
//    npm install sql.js
...
const saveDb = () => {
  try {
    ...
    // Convert Uint8Array → base64 in chunks to avoid stack overflow on large DBs
    let binary = '';
    const chunk = 8192;
    ...
    console.error('[TEA] saveDb error:', e);
  }
};

// ── Load DB bytes from localStorage ─────────────────────────────────────────
const loadDbBytes = () => {
  try {
    ...
    console.error('[TEA] loadDbBytes error:', e);
    return null;
  }
};
...
const sqlAll = (sql, params = []) => {
  try {
    ...
    console.error('[TEA] sqlAll error:', e.message, sql);
    return [];
  }
};
