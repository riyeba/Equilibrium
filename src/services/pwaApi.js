...
export const initDatabase = async () => {
  ...
  initPromise = (async () => {
    ...
    // COMPLETE SCHEMA (Matches your main.js exactly)
    db.exec(`
...
// ─── Query Helpers ───────────────────────────────────────────────
const sqlAll = async (sql, params = []) => { await initDatabase(); const rows = []; db.exec({ sql, bind: params, rowMode: 'object', callback: r => rows.push({ ...r }) }); return rows; };
const sqlGet = async (sql, params = []) => { const rows = await sqlAll(sql, params); return rows[0] || null; };
const sqlRun = async (sql, params = []) => { await initDatabase(); db.exec({ sql, bind: params }); let lastId = null; db.exec({ sql: 'SELECT last_insert_rowid() as id', rowMode: 'object', callback: r => { lastId = r.id; } }); return lastId; };

// ─── pwaApi (Full Missing Parts Added) ──────────────────────────
export const pwaApi = {
  // DASHBOARD (Missing Part Fixed)
  ...
  // AUTH, SETTINGS, STUDENTS... (Already Correct in Previous Turns)
  ...
  // STAFF (Fixed to match Staff.jsx Sequential IDs)
  ...
  // PAYMENTS (Fixed Table Names to student_payments)
  getFees: () => sqlAll("SELECT * FROM fee_types ORDER BY id DESC"),
  getPayments: () => sqlAll(`SELECT p.*, s.first_name, s.last_name FROM student_payments p JOIN students s ON p.student_id = s.id`),
