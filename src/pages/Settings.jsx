    ...
    return (
        <div className='max-w-2xl space-y-6'>
            ...
            <div className='bg-white rounded-2xl border border-gray-200 shadow-sm p-6'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                    {fields.map(([key, label]) => (
                        <div key={key} className={key === 'school_address' || key === 'school_name' ? 'col-span-2' : ''}>
                ...
                <div className='flex justify-end mt-5'>
                    <button onClick={handleSave} disabled={saving}
                        className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all ${saved ? 'bg-green-500 text-white' : 'bg-blue-900 hover:bg-blue-700 text-white'} disabled:opacity-60`}>
            ...
            <div className='bg-white rounded-2xl border border-gray-200 shadow-sm p-6'>
                ...
                {pwError && <div className='bg-red-50 border border-red-200 text-red-700 rounded-xl px-4 py-3 text-sm mb-4'>{pwError}</div>}
                {pwMsg && <div className='bg-green-50 border border-green-200 text-green-700 rounded-xl px-4 py-3 text-sm mb-4'>{pwMsg}</div>}
                    ...
                    {[['current', 'Current Password'], ['newPw', 'New Password'], ['confirm', 'Confirm New Password']].map(([k, label]) => (
                        <div key={k}>
                            ...
                            <div className='relative'>
                                ...
                                <input type='password' className='w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500'
                                    value={pwForm[k]} onChange={e => setPwForm(f => ({ ...f, [k]: e.target.value }))} />
                            </div>
                        </div>
                    ))}
                    <div className='flex justify-end'>
                        <button onClick={handleChangePw} className='flex items-center gap-2 px-6 py-2.5 bg-blue-900 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-colors'>
            ...
            <div className='bg-blue-50 rounded-2xl border border-blue-100 p-5 text-sm text-blue-800'>
                <div className='font-bold mb-2'>System Information</div>
                {/* <div>Logged in as: <strong>{user?.username}</strong> ({user?.role})</div> */}
                <div className='mt-1 text-blue-600 text-xs'>TEA School Management System </div>
            </div>
        </div>
    );
}
